...
Installation and build steps from [Module 01: Digital Operating Room](../01-operating-room/README.md#setup-and-installation) satisfy the prerequisites for this module. Specifically, ensure RTI Connext and the Python API (`rti.connext.activated`) are installed, that the `NDDSHOME` environment variable is set, and that you have installed the Python dependencies from `requirements.txt` into your virtual environment.

### 2. Generate Security Artifacts

First, generate the trusted system security artifacts if not already done. Then generate threat-specific artifacts (rogue CA, forged permissions, etc.). See the [Security README](../../system_arch/security/README.md) for full details on the trusted artifacts.

```bash
# Generate trusted system artifacts:
python3 system_arch/security/setup_security.py

# Generate threat-specific artifacts:
python3 modules/04-security-threat/security/setup_threat_security.py
```

*Note, the **Forged Permissions** and **Expired Certificate** attack modes require the trusted system CA private keys (e.g. `system_arch/security/ca/TrustedIdentityCa/private/TrustedIdentityCa.key`). These keys are generated by `setup_security.py` and are used only for laboratory demonstration of how certificate chains can be abused. Keep them secure in any real deployment.*
